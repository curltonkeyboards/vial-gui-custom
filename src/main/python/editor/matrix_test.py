...
class GamingConfigurator(BasicEditor):
    ...
    def _load_key_mappings(self):
        """Load key mappings from firmware and update button labels"""
        if not self.keyboard:
            return
        try:
            mappings = self.keyboard.get_gaming_key_maps()
            if not mappings:
                return
            ...
            for control_id, mapping in mappings.items():
                if control_id not in self.gaming_controls:
                    continue
                data = self.gaming_controls[control_id]
                if mapping['enabled']:
                    data['row'] = mapping['row']
                    data['col'] = mapping['col']
                    data['enabled'] = True

                    # Reverse-lookup keycode at this (row, col)
                    label = self._find_key_label(mapping['row'], mapping['col'])
                    if label and len(label) > 7:
                        label = label[:6] + ".."
                    data['button'].setText(label or f"R{mapping['row']}C{mapping['col']}")
                else:
                    data['row'] = None
                    data['col'] = None
                    data['enabled'] = False
            ...
            print(f"GamingConfigurator: Error loading key mappings: {e}")

    def _find_key_label(self, row, col):
        """Find the keycode label for a key at (row, col) by searching the keymap"""
        if not self.keyboard:
            return None
        from keycodes.keycodes import Keycode
        # Search layer 0 first, then other layers
        for (layer, r, c), kc in sorted(self.keyboard.layout.items()):
            if r == row and c == col:
                return Keycode.label(kc)
        return None
