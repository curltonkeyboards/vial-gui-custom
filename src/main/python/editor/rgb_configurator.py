...
class HierarchicalComboBox(QComboBox):
    """Custom ComboBox that supports hierarchical menus with hover submenus"""
    
    def __init__(self, effects_list, combo_type="effects", parent=None):
        super().__init__(parent)
        self.effects_list = effects_list
        self.combo_type = combo_type  # "effects" or "backgrounds"
        self.effect_groups = {}
        self.flat_index_to_effect = {}  # Maps flat index to actual effect index
        self.effect_to_flat_index = {}  # Maps effect index to flat index
        self._current_effect_index = 0
        
        self.setup_hierarchical_menu()
        
    def setup_hierarchical_menu(self):
        """Analyze effects and create hierarchical menu structure"""
        if self.combo_type == "backgrounds":
            self.effect_groups = self.group_backgrounds(self.effects_list)
        else:
            self.effect_groups = self.group_effects(self.effects_list)
        self.populate_combo_box()
        
    def group_backgrounds(self, backgrounds_list):
        """Group backgrounds by their prefixes and categories"""
        groups = {}
        
        for i, background in enumerate(backgrounds_list):
            group_name = background
            
            # Determine group based on background name
            if background == "None":
                group_name = "None"
            elif background.startswith("Basic"):
                group_name = "Basic"
            elif background.startswith("Autolight"):
                group_name = "Autolight"
            elif background.startswith("BPM Pulse Fade"):
                group_name = "BPM Pulse Fade"
            elif background.startswith("BPM Quadrants"):
                group_name = "BPM Quadrants"
            elif background.startswith("BPM Row"):
                group_name = "BPM Row"
            elif background.startswith("BPM Column"):
                group_name = "BPM Column"
            elif background.startswith("BPM All"):
                group_name = "BPM All"
        ...
        for group_name in groups:
            if group_name == "Basic":
                # Sort Basic effects: Basic, Basic 2, Basic 3, etc.
                groups[group_name].sort(key=lambda x: (
                    0 if x['name'] == 'Basic' else int(x['name'].split()[-1]) if x['name'].split()[-1].isdigit() else 999
                ))
            elif group_name == "Autolight":
                # Sort Autolight effects: Autolight, Autolight 2, Autolight 3, etc.
                groups[group_name].sort(key=lambda x: (
                    0 if x['name'] == 'Autolight' else int(x['name'].split()[-1]) if x['name'].split()[-1].isdigit() else 999
                ))
            else:
                # Sort BPM effects by their full name
                groups[group_name].sort(key=lambda x: x['name'])
        
        return groups
        
    def group_effects(self, effects_list):
        """Group effects by their base names and variants"""
        groups = {}
        
        # Variant keywords to look for
        variant_keywords = ['Solo', 'Short', 'Long', 'Small', 'Med', 'Large', 'Massive', 
                          'Wide', 'Wider', '2', '3', '4', '5', '6', '1', 'Reverse']
        
        for i, effect in enumerate(effects_list):
            # Find the base name by removing variant keywords
            base_name = effect
            found_variants = []
            ...
            for keyword in variant_keywords:
                if keyword in effect:
                    # Check if it's a separate word or part of a compound
                    pattern = r'\b' + re.escape(keyword) + r'\b'
                    if re.search(pattern, effect):
                        found_variants.append(keyword)
            
            # Create base name by removing variants
            if found_variants:
                # Remove all found variants to get base name
                temp_name = effect
                for variant in found_variants:
                    temp_name = re.sub(r'\b' + re.escape(variant) + r'\b', '', temp_name)
                base_name = re.sub(r'\s+', ' ', temp_name).strip()
                
                # Special handling for numbered variants
                base_name = re.sub(r'\s+\d+\s*
    
    def populate_combo_box(self):
        """Populate the combo box with grouped effects or backgrounds"""
        self.clear()
        flat_index = 0
        
        if self.combo_type == "backgrounds":
            # For backgrounds, organize by logical order
            desired_order = ["None", "Basic", "Autolight", "BPM Pulse Fade", "BPM Quadrants", "BPM Row", "BPM Column", "BPM All"]
            
            for group_name in desired_order:
                if group_name not in self.effect_groups:
                    continue
                    
                effects = self.effect_groups[group_name]
                
                if len(effects) == 1:
                    # Single background (like "None")
                    self.addItem(effects[0]['name'])
                    self.flat_index_to_effect[flat_index] = effects[0]['index']
                    self.effect_to_flat_index[effects[0]['index']] = flat_index
                    flat_index += 1
                else:
                    # Multiple backgrounds in group - add parent item
                    self.addItem(f"{group_name} ▶")
                    group_flat_index = flat_index
                    flat_index += 1
                    
                    # Map the first effect in the group to this index
                    self.flat_index_to_effect[group_flat_index] = effects[0]['index']
                    
                    # Map all effects in the group to this flat index
                    for effect in effects:
                        self.effect_to_flat_index[effect['index']] = group_flat_index
        else:
            # Original effects logic
            # Add single effects first, then grouped effects
            single_effects = []
            grouped_effects = []
            
            for group_name, effects in self.effect_groups.items():
                if len(effects) == 1:
                    single_effects.append((group_name, effects[0]))
                else:
                    grouped_effects.append((group_name, effects))
            
            # Add single effects
            for group_name, effect in single_effects:
                self.addItem(effect['name'])
                self.flat_index_to_effect[flat_index] = effect['index']
                self.effect_to_flat_index[effect['index']] = flat_index
                flat_index += 1
            
            # Add grouped effects with submenus
            for group_name, effects in grouped_effects:
                # Add a parent item for the group
                self.addItem(f"{group_name} ▶")
                group_flat_index = flat_index
                flat_index += 1
    ...
    def showPopup(self):
        """Override to show custom hierarchical menu"""
        menu = QMenu(self)
        
        # Track current flat index
        flat_index = 0
        
        # Add single effects first
        single_effects = []
        grouped_effects = []
        ...
        # Add single effects to menu
        for group_name, effect in single_effects:
            action = QAction(effect['name'], menu)
            action.triggered.connect(lambda checked, idx=flat_index: self.select_effect(idx))
            menu.addAction(action)
            flat_index += 1
        
        if single_effects and grouped_effects:
            menu.addSeparator()
        ...
        for group_name, effects in grouped_effects:
            ...
            for effect in effects:
                action = QAction(effect['name'], group_menu)
                action.triggered.connect(lambda checked, idx=group_flat_index, eff_idx=effect['index']: 
                                       self.select_grouped_effect(idx, eff_idx))
    ...
    def select_effect(self, flat_index):
        """Handle selection of a single effect"""
        if flat_index in self.flat_index_to_effect:
            effect_index = self.flat_index_to_effect[flat_index]
            self._current_effect_index = effect_index
            self.setCurrentIndex(flat_index)
            self.currentIndexChanged.emit(effect_index)
    
    def select_grouped_effect(self, flat_index, effect_index):
        """Handle selection of an effect from a group"""
        self._current_effect_index = effect_index
        # Update the display to show the selected effect
        self.setItemText(flat_index, self.effects_list[effect_index])
        self.setCurrentIndex(flat_index)
        # Update the mapping
        self.flat_index_to_effect[flat_index] = effect_index
        self.currentIndexChanged.emit(effect_index)
    
    def setCurrentEffectIndex(self, effect_index):
        """Set the current selection by effect index"""
        if effect_index < len(self.effects_list):
            self._current_effect_index = effect_index
            if effect_index in self.effect_to_flat_index:
                flat_index = self.effect_to_flat_index[effect_index]
                
                # Update display text for grouped items
                if flat_index in self.flat_index_to_effect:
                    current_mapped = self.flat_index_to_effect[flat_index]
                    if current_mapped != effect_index:
                        # This is a grouped item, update the display
                        self.setItemText(flat_index, self.effects_list[effect_index])
                        self.flat_index_to_effect[flat_index] = effect_index
                
                self.setCurrentIndex(flat_index)
    
    def currentEffectIndex(self):
        """Get the current effect index"""
        return self._current_effect_index
...
VIALRGB_EFFECTS = [
    ...
    VialRGBEffect(33, "Solid Reactive 2"),
...
LIVE_EFFECTS = [
    ...
    "2",                                             # LIVE_ANIM_WIDE1
    "2 Solo",                                        # LIVE_ANIM_WIDE1_SOLO
    "2r",                                            # LIVE_ANIM_WIDE2
    "2r Solo",                                       # LIVE_ANIM_WIDE2_SOLO
    ...
    "Volume Column Small 2",                        # LIVE_ANIM_VOLUME_UP_DOWN_1_WIDE
    "Volume Column Small 2 Solo",                   # LIVE_ANIM_VOLUME_UP_DOWN_1_WIDE_SOLO
    "Volume Column Large",                             # LIVE_ANIM_VOLUME_UP_DOWN_2
    "Volume Column Large Solo",                        # LIVE_ANIM_VOLUME_UP_DOWN_2_SOLO
    "Volume Column Large 2",                        # LIVE_ANIM_VOLUME_UP_DOWN_2_WIDE
    "Volume Column Large 2 Solo",                   # LIVE_ANIM_VOLUME_UP_DOWN_2_WIDE_SOLO
    "Volume Row Small",                          # LIVE_ANIM_VOLUME_LEFT_RIGHT_1
    "Volume Row Small Solo",                     # LIVE_ANIM_VOLUME_LEFT_RIGHT_1_SOLO
    "Volume Row Small 2",                     # LIVE_ANIM_VOLUME_LEFT_RIGHT_1_WIDE
    "Volume Row Small 2 Solo",                # LIVE_ANIM_VOLUME_LEFT_RIGHT_1_WIDE_SOLO
    "Volume Row Med",                            # LIVE_ANIM_VOLUME_LEFT_RIGHT_2
    "Volume Row Med Solo",                       # LIVE_ANIM_VOLUME_LEFT_RIGHT_2_SOLO
    "Volume Row Med 2",                       # LIVE_ANIM_VOLUME_LEFT_RIGHT_2_WIDE
    "Volume Row Med 2 Solo",                  # LIVE_ANIM_VOLUME_LEFT_RIGHT_2_WIDE_SOLO
    "Volume Row Large",                          # LIVE_ANIM_VOLUME_LEFT_RIGHT_3
    "Volume Row Large Solo",                     # LIVE_ANIM_VOLUME_LEFT_RIGHT_3_SOLO
    "Volume Row Large 2",                     # LIVE_ANIM_VOLUME_LEFT_RIGHT_3_WIDE
    "Volume Row Large 2 Solo",                # LIVE_ANIM_VOLUME_LEFT_RIGHT_3_WIDE_SOLO
    "Collapsing Column Small",                        # LIVE_ANIM_PEAK_VOLUME_UP_DOWN_1
    "Collapsing Column Small Solo",                   # LIVE_ANIM_PEAK_VOLUME_UP_DOWN_1_SOLO
    "Collapsing Column Small 2",                   # LIVE_ANIM_PEAK_VOLUME_UP_DOWN_1_WIDE
    "Collapsing Column Small 2 Solo",              # LIVE_ANIM_PEAK_VOLUME_UP_DOWN_1_WIDE_SOLO
    "Collapsing Column Large",                        # LIVE_ANIM_PEAK_VOLUME_UP_DOWN_2
    "Collapsing Column Large Solo",                   # LIVE_ANIM_PEAK_VOLUME_UP_DOWN_2_SOLO
    "Collapsing Column Large 2",                   # LIVE_ANIM_PEAK_VOLUME_UP_DOWN_2_WIDE
    "Collapsing Column Large 2 Solo",              # LIVE_ANIM_PEAK_VOLUME_UP_DOWN_2_WIDE_SOLO
    "Collapsing Row Small",                     # LIVE_ANIM_PEAK_VOLUME_LEFT_RIGHT_1
    "Collapsing Row Small Solo",                # LIVE_ANIM_PEAK_VOLUME_LEFT_RIGHT_1_SOLO
    "Collapsing Row Small 2",                # LIVE_ANIM_PEAK_VOLUME_LEFT_RIGHT_1_WIDE
    "Collapsing Row Small 2 Solo",           # LIVE_ANIM_PEAK_VOLUME_LEFT_RIGHT_1_WIDE_SOLO
    "Collapsing Row Med",                       # LIVE_ANIM_PEAK_VOLUME_LEFT_RIGHT_2
    "Collapsing Row Med Solo",                  # LIVE_ANIM_PEAK_VOLUME_LEFT_RIGHT_2_SOLO
    "Collapsing Row Med 2",                  # LIVE_ANIM_PEAK_VOLUME_LEFT_RIGHT_2_WIDE
    "Collapsing Row Med 2 Solo",             # LIVE_ANIM_PEAK_VOLUME_LEFT_RIGHT_2_WIDE_SOLO
    "Collapsing Row Large",                     # LIVE_ANIM_PEAK_VOLUME_LEFT_RIGHT_3
    "Collapsing Row Large Solo",                # LIVE_ANIM_PEAK_VOLUME_LEFT_RIGHT_3_SOLO
    "Collapsing Row Large 2",                # LIVE_ANIM_PEAK_VOLUME_LEFT_RIGHT_3_WIDE
    "Collapsing Row Large 2 Solo"            # LIVE_ANIM_PEAK_VOLUME_LEFT_RIGHT_3_WIDE_SOLO
]
...
class CustomLightsHandler(BasicHandler):
    ...
    def create_slot_tab(self, slot):
        """Create a tab for a single slot using hierarchical combo boxes"""
        # Create tab widget
        tab_widget = QWidget()
        ...
        # Live Effect with hierarchical menu
        layout.addWidget(QLabel(tr("RGBConfigurator", "Effect:")), 1, 0)
        live_effect = HierarchicalComboBox(LIVE_EFFECTS)
        ...
        # Macro Effect with hierarchical menu
        layout.addWidget(QLabel(tr("RGBConfigurator", "Effect:")), 5, 0)
        macro_effect = HierarchicalComboBox(MACRO_EFFECTS)
            ...
            try:
                if hasattr(self.device.keyboard, 'get_custom_slot_config'):
                    ...
                    if config and len(config) >= 12:  # Expecting 12 parameters
                        ...
                        # Set hierarchical combo boxes using their special methods
                        widgets['live_effect'].setCurrentEffectIndex(min(config[2], len(LIVE_EFFECTS) - 1))  # live_animation
                        widgets['live_style'].setCurrentIndex(min(config[0], len(LIVE_STYLES) - 1))    # live_positioning
                        widgets['macro_effect'].setCurrentEffectIndex(min(config[3], len(MACRO_EFFECTS) - 1)) # macro_animation
    ...
    def set_slot_defaults(self, slot):
        ...
        widgets = self.slot_widgets[slot]
        widgets['live_effect'].setCurrentEffectIndex(0)         # None
        widgets['live_style'].setCurrentIndex(0)          # TrueKey
        widgets['live_speed'].setValue(128)               # Default live speed
        widgets['macro_effect'].setCurrentEffectIndex(0)        # None
    ...
    def on_live_effect_changed(self, slot, effect_index):
        """Handle live effect change using hierarchical combo box effect index"""
        if hasattr(self.device.keyboard, 'set_custom_slot_parameter'):
            self.device.keyboard.set_custom_slot_parameter(slot, 2, effect_index)  # live_animation
        else:
            print(f"Live effect changed: slot {slot}, effect {effect_index}")
    ...
    def on_macro_effect_changed(self, slot, effect_index):
        """Handle macro effect change using hierarchical combo box effect index"""
        if hasattr(self.device.keyboard, 'set_custom_slot_parameter'):
            self.device.keyboard.set_custom_slot_parameter(slot, 3, effect_index)  # macro_animation
        else:
            print(f"Macro effect changed: slot {slot}, effect {effect_index}")
    ...
    def on_load_preset(self, slot, index):
        ...
        presets = [
            ...
            {'live_pos': 0, 'live_anim': 6, 'macro_pos': 0, 'macro_anim': 6, 'background': 1, 'sustain': 3, 'color': 3, 'bg_brightness': 25, 'live_speed': 200, 'macro_speed': 200},
            # Moving Dots: live(Zone,Moving Dots Row), macro(Zone,Moving Dots Row), Basic bg, All sustain, Channel color
            {'live_pos': 1, 'live_anim': 16, 'macro_pos': 1, 'macro_anim': 16, 'background': 1, 'sustain': 3, 'color': 1, 'bg_brightness': 35, 'live_speed': 150, 'macro_speed': 150},
            # BPM Disco: live(Quadrant,None), macro(Quadrant,None), BPM All Disco bg, All sustain, Macro color
            {'live_pos': 2, 'live_anim': 0, 'macro_pos': 2, 'macro_anim': 0, 'background': 56, 'sustain': 3, 'color': 2, 'bg_brightness': 40, 'live_speed': 100, 'macro_speed': 100},
            ...
            # Sustain Mode: live(TrueKey,Heatmap 2), macro(TrueKey,Heatmap 2), None bg, All sustain, Channel color
            {'live_pos': 0, 'live_anim': 7, 'macro_pos': 0, 'macro_anim': 7, 'background': 0, 'sustain': 3, 'color': 1, 'bg_brightness': 0, 'live_speed': 80, 'macro_speed': 80},
            # Performance Setup: live(TrueKey,None), macro(Zone,Heatmap), Basic bg, All sustain, Channel color
            {'live_pos': 0, 'live_anim': 0, 'macro_pos': 1, 'macro_anim': 6, 'background': 1, 'sustain': 3, 'color': 1, 'bg_brightness': 30, 'live_speed': 180, 'macro_speed': 180},
        ]
        ...
        try:
            ...
            if hasattr(self.device.keyboard, 'set_custom_slot_all_parameters'):
                ...
                if success:
                    ...
                    widgets = self.slot_widgets[slot]
                    widgets['live_effect'].setCurrentEffectIndex(preset['live_anim'])
                    widgets['live_style'].setCurrentIndex(preset['live_pos'])
                    widgets['macro_effect'].setCurrentEffectIndex(preset['macro_anim'])
...
class RGBConfigurator(BasicEditor):
    ...
    def rebuild(self, device):
        ...
        self.update_from_keyboard(), '', base_name).strip()
        ...
        for group_name in groups:
            groups[group_name].sort(key=lambda x: (
                'Solo' not in x['name'],  # Non-solo first
    ...
    def populate_combo_box(self):
        """Populate the combo box with grouped effects"""
        self.clear()
        flat_index = 0
