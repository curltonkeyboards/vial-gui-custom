...
class LightingTab2(QScrollArea):
    ...
    def __init__(self, parent, label, inversion_keycodes, inversion_keycodes4, smartchord_LSB, smartchord_MSB, smartchord_LSB2):
        ...
        # Row 1: Layer Mode dropdowns
        self.row1_layout = QHBoxLayout()
        self.row1_layout.addStretch()  # Left spacer

        # Create and add dropdowns with fixed width
        self.layer_mode1_dropdown = self.create_layer_mode1_dropdown()
        self.layer_mode1_dropdown.setFixedWidth(200)
        self.row1_layout.addWidget(self.layer_mode1_dropdown)

        self.layer_mode2_dropdown = self.create_layer_mode2_dropdown()
        self.layer_mode2_dropdown.setFixedWidth(200)
        self.row1_layout.addWidget(self.layer_mode2_dropdown)
        ...
        # Add "Additional Layer Controls" label
        self.layer_lighting_label = QLabel("Additional Layer Controls")
        self.layer_lighting_label.setAlignment(Qt.AlignCenter)
        self.layer_lighting_label.setStyleSheet("font-size: 13px;")
        self.main_layout.addWidget(self.layer_lighting_label)
        ...
        # Row 3: Layer Mode 3 dropdown and buttons from inversion_keycodes4
        self.row3_layout = QHBoxLayout()
        self.row3_layout.addStretch()  # Left spacer

        # Add Layer Mode 3 dropdown
        self.layer_mode3_dropdown = self.create_layer_mode3_dropdown()
        self.layer_mode3_dropdown.setFixedWidth(200)
        self.row3_layout.addWidget(self.layer_mode3_dropdown)
    ...
    def create_layer_mode1_dropdown(self):
        dropdown = ScrollableComboBox()
        dropdown.setFixedHeight(40)
    ...
    def create_layer_mode2_dropdown(self):
        dropdown = ScrollableComboBox()
        dropdown.setFixedHeight(40)
    ...
    def create_layer_mode3_dropdown(self):
        dropdown = ScrollableComboBox()
        dropdown.setFixedHeight(40)
        dropdown.addItem("Tap-Toggle Layer")
        for keycode in self.smartchord_LSB2:
            label = Keycode.label(keycode.qmk_id)
            tooltip = Keycode.description(keycode.qmk_id)
            dropdown.addItem(label, keycode.qmk_id)
        ...
        dropdown.model().item(0).setEnabled(False)
        dropdown.currentIndexChanged.connect(self.on_selection_change)
        dropdown.currentIndexChanged.connect(lambda _: self.reset_dropdown(dropdown, "Tap-Toggle Layer"))
        return dropdown
    ...
    def recreate_buttons(self, keycode_filter=None):
        # Clear and recreate the dropdowns in row 1
        self.row1_layout.removeWidget(self.layer_mode1_dropdown)
        self.row1_layout.removeWidget(self.layer_mode2_dropdown)

        self.layer_mode1_dropdown.deleteLater()
        self.layer_mode2_dropdown.deleteLater()

        self.layer_mode1_dropdown = self.create_layer_mode1_dropdown()
        self.layer_mode1_dropdown.setFixedWidth(200)
        self.row1_layout.insertWidget(1, self.layer_mode1_dropdown)

        self.layer_mode2_dropdown = self.create_layer_mode2_dropdown()
        self.layer_mode2_dropdown.setFixedWidth(200)
        self.row1_layout.insertWidget(2, self.layer_mode2_dropdown)

        # Clear and recreate the dropdown in row 3
        self.row3_layout.removeWidget(self.layer_mode3_dropdown)
        self.layer_mode3_dropdown.deleteLater()

        self.layer_mode3_dropdown = self.create_layer_mode3_dropdown()
        self.layer_mode3_dropdown.setFixedWidth(200)
        self.row3_layout.insertWidget(1, self.layer_mode3_dropdown)
