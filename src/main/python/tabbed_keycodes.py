...
class GamingTab(QScrollArea):
    ...
    def recreate_buttons(self, keycode_filter=None):
        ...
        # Triggers (LT and RT) - at top (moved 80px right and left respectively, moved up 40px)
        lt_btn = self.create_button("LT", 60, 35)
        if lt_btn:
            lt_btn.setParent(gamepad_widget)
            lt_btn.move(200, 40)  # Top left - moved up 40px from 80

        rt_btn = self.create_button("RT", 60, 35)
        if rt_btn:
            rt_btn.setParent(gamepad_widget)
            rt_btn.move(490, 40)  # Top right - moved up 40px from 80
        ...
        # D-pad up: wider at bottom (30px), tapers to curved point at top
        dpad_up = self.create_button("DPAD_UP", 30, 52)
        if dpad_up:
            dpad_up.setText("↑")
            dpad_up.setParent(gamepad_widget)
            dpad_up.move(210, 143)  # Moved up 60px from 203
        ...
        # D-pad down: wider at top (30px), tapers to curved point at bottom
        dpad_down = self.create_button("DPAD_DOWN", 30, 52)
        if dpad_down:
            dpad_down.setText("↓")
            dpad_down.setParent(gamepad_widget)
            dpad_down.move(210, 207)  # Moved up 60px from 267
            path = QPainterPath()
        ...
        # D-pad left: wider at right (30px), tapers to curved point at left
        dpad_left = self.create_button("DPAD_LEFT", 52, 30)
        if dpad_left:
            dpad_left.setText("←")
            dpad_left.setParent(gamepad_widget)
            dpad_left.move(169, 180)  # Moved up 60px from 240
            path = QPainterPath()
        ...
        # D-pad right: wider at left (30px), tapers to curved point at right
        dpad_right = self.create_button("DPAD_RIGHT", 52, 30)
        if dpad_right:
            dpad_right.setText("→")
            dpad_right.setParent(gamepad_widget)
            dpad_right.move(243, 180)  # Moved up 60px from 240
            path = QPainterPath()
        ...
        if btn3:
            ...
            btn3.move(473, 169)  # Left - moved up 60px from 229
        ...
        if btn2:
            ...
            btn2.move(537, 169)  # Right - moved up 60px from 229
        ...
        if btn1:
            ...
            btn1.move(505, 201)  # Bottom - moved up 60px from 261
