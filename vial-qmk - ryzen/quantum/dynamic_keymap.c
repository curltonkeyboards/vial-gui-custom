...
uint16_t macro_positions[256] = {0};
bool positions_cached = false;
...
void cache_macro_positions(void) {
    void *p = (void *)(DYNAMIC_KEYMAP_MACRO_EEPROM_ADDR);
    void *end = (void *)(DYNAMIC_KEYMAP_MACRO_EEPROM_ADDR + DYNAMIC_KEYMAP_MACRO_EEPROM_SIZE);
    uint16_t pos = 0;

    // Initialize all positions
    for (int i = 0; i < DYNAMIC_KEYMAP_MACRO_COUNT; i++) {
        macro_positions[i] = pos;
        
        // Skip to end of current macro (0 byte)
        while (p < end && eeprom_read_byte(p) != 0) {
            p++;
            pos++;
        }
        // Move past the terminating 0
        if (p < end) {
            p++;
            pos++;
        }
...
void update_macro_positions(uint8_t id) {
    if (!positions_cached) cache_macro_positions();
    
    // Update positions for all macros after the modified one
    void *p = (void *)(DYNAMIC_KEYMAP_MACRO_EEPROM_ADDR + macro_positions[id]);
    uint16_t pos = macro_positions[id];
    
    while (id < DYNAMIC_KEYMAP_MACRO_COUNT - 1) {
        while (eeprom_read_byte(p++) != 0) pos++;
        macro_positions[++id] = ++pos;
    }
}
...
void dynamic_keymap_macro_set_buffer(uint16_t offset, uint16_t size, uint8_t *data) {
    ...
    // Recache positions when macro data changes
            ...
            if (data[1] == SS_TAP_CODE || data[1] == SS_DOWN_CODE || data[1] == SS_UP_CODE) {
                ...
                if (data[2] != 0) {
                    ...
                    if (data[3] != 0) {
                        ...
                        switch (data[1]) {
                            case VIAL_MACRO_EXT_TAP: vial_keycode_tap(kc); break;
                            case VIAL_MACRO_EXT_DOWN: vial_keycode_down(kc); break;
                            case VIAL_MACRO_EXT_UP: vial_keycode_up(kc); break;
                        }
                    }
                }
