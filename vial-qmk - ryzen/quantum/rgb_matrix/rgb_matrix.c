...
//   1 channel at 255: 877mA -> need 91% scale (233/256) for 800mA
//   2 channels at 255: 1127mA -> need 71% scale (182/256) for 800mA
//   3 channels at 255: 1120mA -> need 71% scale (182/256) for 800mA
//
...
static void apply_brightness_scaling_to_rgb(uint8_t *red, uint8_t *green, uint8_t *blue) {
    ...
    // Smoothly interpolate scale factor between single-channel (233) and
    // multi-channel (182) based on secondary channel contribution.
    // secondary=0   -> scale=233 (91%, single channel power draw)
    // secondary>=255 -> scale=182 (71%, multi-channel power draw plateau)
    uint8_t scale;
    if (secondary == 0) {
        scale = 233;
        ...
        // Linear interpolation: 233 - (233-182) * secondary / 255
        scale = 233 - (uint8_t)(((uint16_t)51 * secondary) / 255);
    }
...
static uint8_t rgb_matrix_get_effective_mode(void) {
    return rgb_matrix_config.bank * 64 + rgb_matrix_config.mode;
}

static void rgb_matrix_set_effective_mode(uint8_t effective_mode) {
    rgb_matrix_config.bank = effective_mode / 64;
    rgb_matrix_config.mode = effective_mode % 64;
}

static uint8_t rgb_matrix_get_max_mode(void) {
    // Update this number to your total number of effects
    // For your BPM effects, this should be around 83 (0-based counting)
    return 128;
}
// double buffers
...
void rgb_matrix_set_color(int index, uint8_t red, uint8_t green, uint8_t blue) {
    // Apply brightness scaling to direct RGB values too
    apply_brightness_scaling_to_rgb(&red, &green, &blue);
...
void rgb_matrix_set_color_all(uint8_t red, uint8_t green, uint8_t blue) {
    ...
    for (uint8_t i = 0; i < RGB_MATRIX_LED_COUNT; i++)
        rgb_matrix_driver.set_color(i, red, green, blue);
#else
...
// REPLACE the existing rgb_task_render function with this:
static void rgb_task_render(uint8_t effect) {
    ...
    // Get the effective effect number for the switch statement
    uint8_t effective_effect = effect;
    
    // If effect is in bank 1+, map it to handle after the original switch
    if (effect >= RGB_MATRIX_EFFECT_MAX) {
        effective_effect = RGB_MATRIX_EFFECT_MAX - 1;
    }

    // each effect can opt to do calculations
    // and/or request PWM buffer updates.
    switch (effective_effect) {
...
void rgb_matrix_task(void) {
	...
	uint8_t effect = suspend_backlight || !rgb_matrix_config.enable ? 0 : rgb_matrix_get_effective_mode();
...
// REPLACE the existing rgb_matrix_mode_eeprom_helper function with this:
void rgb_matrix_mode_eeprom_helper(uint8_t mode, bool write_to_eeprom) {
    if (!rgb_matrix_config.enable) {
        return;
    }
    uint8_t max_mode = rgb_matrix_get_max_mode();
    if (mode < 1) {
        rgb_matrix_set_effective_mode(1);
    } else if (mode > max_mode) {
        rgb_matrix_set_effective_mode(max_mode);
    } else {
        rgb_matrix_set_effective_mode(mode);
    }
    rgb_task_state = STARTING;
    eeconfig_flag_rgb_matrix(write_to_eeprom);
    dprintf("rgb matrix mode [%s]: %u (bank:%u mode:%u)\n", 
        (write_to_eeprom) ? "EEPROM" : "NOEEPROM", 
        rgb_matrix_get_effective_mode(), rgb_matrix_config.bank, rgb_matrix_config.mode);
}
...
uint8_t rgb_matrix_get_mode(void) {
    return rgb_matrix_get_effective_mode();
}

// REPLACE the existing rgb_matrix_step_helper function with this:
void rgb_matrix_step_helper(bool write_to_eeprom) {
    uint8_t current_mode = rgb_matrix_get_effective_mode();
    uint8_t next_mode = current_mode + 1;
    uint8_t max_mode = rgb_matrix_get_max_mode();
    rgb_matrix_mode_eeprom_helper((next_mode > max_mode) ? 1 : next_mode, write_to_eeprom);
}
...
void rgb_matrix_step_reverse_helper(bool write_to_eeprom) {
    uint8_t current_mode = rgb_matrix_get_effective_mode();
    uint8_t prev_mode = current_mode - 1;
    uint8_t max_mode = rgb_matrix_get_max_mode();
    rgb_matrix_mode_eeprom_helper((prev_mode < 1) ? max_mode : prev_mode, write_to_eeprom);
}
