        ...
        if (state->retrigger_eligible) {
            if (travel > prev_last_travel) {
                ...
                if (repress_distance >= retrigger_travel || travel >= RETRIGGER_MAX_TRAVEL) {
                    ...
                    // Pass raw_velocity (velocity mode output) for quick build recording
                    midi_send_noteoff_with_recording(state->note_channel, state->midi_note, 127, 0, state->zone_type);
                    midi_send_noteon_with_recording(state->note_channel, state->midi_note, velocity, state->raw_velocity, state->zone_type);
