...
bool effect_runner_reactive(effect_params_t* params, reactive_f effect_func) {
    ...
    for (uint8_t i = led_min; i < led_max; i++) {
        ...
        // Check if this LED or the LED 2 spaces to the left was recently hit
        bool hit = false;
        for (int8_t j = g_last_hit_tracker.count - 1; j >= 0; j--) {
            if ((g_last_hit_tracker.index[j] == i || g_last_hit_tracker.index[j] == i - 2) && g_last_hit_tracker.tick[j] < tick) {
