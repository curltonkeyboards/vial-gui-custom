...
bool effect_runner_reactive_splash(uint8_t start, effect_params_t* params, reactive_splash_f effect_func) {
    ...
    for (uint8_t i = led_min; i < led_max; i++) {
        ...
        for (uint8_t j = start; j < count; j++) {
            // Calculate the distance from the LED to the last hit key
            int16_t  dx   = g_led_config.point[i].x - g_last_hit_tracker.x[j];
            int16_t  dy   = g_led_config.point[i].y - g_last_hit_tracker.y[j];
            uint8_t  dist = sqrt16(dx * dx + dy * dy);
            uint16_t tick = scale16by8(g_last_hit_tracker.tick[j], led_matrix_eeconfig.speed);

            // Check if the LED is the pressed key or the key 2 spaces to the right
            if ((dx == 0 && dy == 0) || (dx == 32 && dy == 0)) { // Assuming each key is 16 units apart
