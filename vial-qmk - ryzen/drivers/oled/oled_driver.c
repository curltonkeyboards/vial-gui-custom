...
// DMA state tracking
#if defined(OLED_USE_DMA) && defined(OLED_TRANSPORT_SPI)
static volatile bool oled_dma_in_progress = false;
#endif
...
#if defined(OLED_TRANSPORT_SPI)
#    if defined(OLED_USE_DMA)
    // Wait for any previous DMA transfer to complete
    if (oled_dma_in_progress) {
        spi_wait_complete();
        oled_dma_in_progress = false;
        spi_stop();
    }
#    endif
...
#    if defined(OLED_USE_DMA)
    ...
    if (spi_transmit_async(data, size) != SPI_STATUS_SUCCESS) {
        spi_stop();
        return false;
    }
    oled_dma_in_progress = true;
    // Return immediately - transfer continues in background
    // spi_stop() will be called at start of next transfer or in oled_flush_dma()
#    else
...
#if defined(OLED_USE_DMA) && defined(OLED_TRANSPORT_SPI)
// Force completion of any pending DMA transfer
...
#if defined(OLED_USE_DMA) && defined(OLED_TRANSPORT_SPI)
    // Ensure DMA transfer completes before turning off
    oled_flush_dma();
#endif
