...
// BPM_ROW_DISCO Effect
RGB_MATRIX_EFFECT(BPM_ROW_DISCO)
static bool bpm_row_disco_runner(effect_params_t* params) {
    ...
    // Render the pulse across entire matrix, but only light the active rows
    for (uint8_t row = 0; row < 5; row++) {
        for (uint8_t col = 0; col < 14; col++) {
            ...
            if (led_count > 0) {
                ...
                if (pulse_intensity > 0 && in_active_rows) {
                    ...
                    // Turn off LED when not pulsing or not in active rows
...
bool BPM_ROW_DISCO(effect_params_t* params) {
    return bpm_row_disco_runner(params);
}
