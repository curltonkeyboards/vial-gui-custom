...
RGB_MATRIX_EFFECT(TETRIS_V)

static bool TETRIS_V_runner(effect_params_t* params) {
    static uint8_t grid[5][14] = {0}; // 5 rows, 14 cols
    static uint8_t drop_timer = 0;
    static uint8_t current_x = 7;
    ...
    if (++drop_timer > (60 - (rgb_matrix_config.speed / 4))) { // Scale with RGB speed
        ...
        if (current_y < 4 && grid[current_y + 1][current_x] == 0) {
            ...
            current_y = 0;
            current_hue = rand() % 255;
            if (grid[0][current_x] == 0) {
                grid[current_y][current_x] = current_hue;
            } else {
                // Top row is full, reset the game
                memset(grid, 0, sizeof(grid));
                current_x = 7;
                current_y = 0;
                current_hue = rand() % 255;
...
bool TETRIS_V(effect_params_t* params) {
    return TETRIS_V_runner(params);
}