...
void add_lighting_macro_note(uint8_t channel, uint8_t note, uint8_t track_id, uint8_t velocity) {
    (void)velocity; // Not used in split version
...
static void render_bpm_background(background_mode_t background_mode, uint8_t background_brightness_pct, led_output_t* output_buffer) {
    uint8_t base_hue = rgb_matrix_get_hue();
    uint8_t base_sat = rgb_matrix_get_sat();
    ...
    if (background_mode >= BACKGROUND_BPM_PULSE_FADE && background_mode <= BACKGROUND_BPM_ALL_DISCO) {
        ...
        if (!is_disco) {
            switch (variant) {
                case 1: pulse_hue = (base_hue + 64) % 256; break;
                case 2: pulse_hue = (base_hue + 128) % 256; break;
                case 3: pulse_hue = (base_hue + 192) % 256; break;
                case 4: pulse_sat = base_sat / 2; break;
                case 5:
                    ...
                    if (led_count > 0) {
                        ...
                        if (in_active_area) {
                            if (is_disco) {
                                ...
                                output_buffer[led[0]].hue = pulse_hue;
                                output_buffer[led[0]].sat = pulse_sat;
                                output_buffer[led[0]].brightness = brightness_factor;
                                output_buffer[led[0]].active = true;
                            }
                        }
                    }
                }
...
static void composite_and_render(background_mode_t background_mode, uint8_t background_brightness_pct) {
    ...
    // First, render background (live system handles this)
    if (background_mode >= BACKGROUND_BPM_PULSE_FADE && background_mode <= BACKGROUND_BPM_ALL_DISCO) {
        // BPM backgrounds are rendered into live_output buffer
        render_bpm_background(background_mode, background_brightness_pct, live_output);
    ...
    for (uint8_t i = 0; i < RGB_MATRIX_LED_COUNT; i++) {
        if (live_output[i].active) {
            ...
            // Only turn off LEDs if background is NONE and no effects active
            rgb_matrix_set_color(i, 0, 0, 0);
        }
        // For static/BPM/autolight backgrounds, background is already rendered, don't overwrite
    }
}
...
static void render_live_moving_dots(background_mode_t background_mode, uint8_t background_brightness_pct) {
    ...
    // Render background into live output if BPM mode
    if (background_mode >= BACKGROUND_BPM_PULSE_FADE && background_mode <= BACKGROUND_BPM_ALL_DISCO) {
        render_bpm_background(background_mode, background_brightness_pct, live_output);
    }
...
static void render_macro_moving_dots(background_mode_t background_mode, uint8_t background_brightness_pct) {
    uint8_t base_hue = rgb_matrix_get_hue();
    uint8_t base_sat = rgb_matrix_get_sat();
...
static void render_live_decay_effects(void) {
    uint8_t base_hue = rgb_matrix_get_hue();
    uint8_t base_sat = rgb_matrix_get_sat();
...
static void render_live_heat_effects(void) {
    uint8_t base_sat = rgb_matrix_get_sat();
...
static bool run_separated_effect(effect_params_t* params,
    ...
    if (live_moving_dots_mode) {
        ...
        render_live_decay_effects();
    } else {
        render_live_heat_effects();
    }
    
    if (macro_moving_dots_mode) {
        render_macro_moving_dots(background_mode, background_brightness_pct);
