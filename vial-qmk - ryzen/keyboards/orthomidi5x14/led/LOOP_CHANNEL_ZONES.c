// LOOP_CHANNEL_ZONES Effect - Zones triggered by live MIDI channels
#include "process_midi.h"
RGB_MATRIX_EFFECT(LOOP_CHANNEL_ZONES)

// CHANNEL_ZONES Effect - Zones colored by channel
#include "process_midi.h"
RGB_MATRIX_EFFECT(CHANNEL_ZONES)
...
static int8_t find_tracked_note_zones(uint8_t channel, uint8_t note, bool is_macro) {
...
static bool add_tracked_note_zones(uint8_t channel, uint8_t note, bool is_macro) {
    for (uint8_t i = 0; i < MAX_TRACKED_NOTES_ZONES; i++) {
        if (!tracked_notes_zones[i].active) {
            tracked_notes_zones[i].channel = channel;
            tracked_notes_zones[i].note = note;
...
static bool channel_zones_runner(effect_params_t* params) {
    static uint8_t zone_map[5][14]; // Store channel for each key position
    static uint8_t zone_brightness[5][14]; // Brightness for each zone
    static uint32_t last_update_time = 0;
    
    // Channel hue offsets from base color (same as original channel waves)
    ...
    for (uint8_t i = 0; i < macro_note_count; i++) {
        ...
        // macro_id is ignored for coloring - we use channel instead
        
        // Check if this macro note is already tracked
        if (find_tracked_note_zones(channel, note, true) == -1) {
            // New macro note detected - trigger animation and track it
            uint8_t row = (note / 12) % 5;
            uint8_t col = (note % 12) + 1;
            if (col >= 14) col = 13;
            
            // Use channel for coloring (not macro_id)
            uint8_t color_channel = channel % 16; // Limit to available colors
            
            // Assign zone and set brightness
            zone_map[row][col] = color_channel;
                ...
                for (int8_t dc = -1; dc <= 1; dc++) {
                    ...
                    if (new_row >= 0 && new_row < 5 && new_col >= 0 && new_col < 14) {
                        if (zone_brightness[new_row][new_col] < 128) {
                            zone_map[new_row][new_col] = color_channel;
            ...
            add_tracked_note_zones(channel, note, true);
        }
    }
    ...
    for (uint8_t i = 0; i < live_note_count; i++) {
        ...
        if (find_tracked_note_zones(channel, note, false) == -1) {
            // New live note detected - trigger animation and track it
            uint8_t row = (note / 12) % 5;
            uint8_t col = (note % 12) + 1;
            if (col >= 14) col = 13;
            
            // Use channel for coloring (same as macro notes)
            uint8_t color_channel = channel % 16; // Limit to available colors
            ...
            add_tracked_note_zones(channel, note, false);
        }
    }
    ...
    for (uint8_t row = 0; row < 5; row++) {
        for (uint8_t col = 0; col < 14; col++) {
            ...
            if (led_count > 0) {
                uint8_t color_channel = zone_map[row][col];
                uint8_t brightness = zone_brightness[row][col];
                
                if (color_channel < 16 && brightness > 0) {
                    // Calculate HSV based on base color and channel offset
                    HSV hsv;
                    hsv.h = (base_hue + channel_hue_offsets[color_channel]) % 256;
                    hsv.s = base_sat;
...
bool LOOP_CHANNEL_ZONES(effect_params_t* params) {
    return channel_zones_runner(params);
}